"""Integration tests for ArborMCPServer base class.
...
from hr_advisory.mcp_servers.base import ArborMCPServer, TenantContext, ToolInvocationRecord
...
class TestToolRegistration:
    ...
    def test_registered_tools_appear_in_list(self, mcp_server: ArborMCPServer):
    ...
    def test_tool_description_preserved(self, mcp_server: ArborMCPServer):
    ...
    def test_requires_confirmation_flag_stored(self, mcp_server: ArborMCPServer):
    ...
    def test_unknown_tool_returns_error(self, mcp_server: ArborMCPServer):
        """Calling an unregistered tool returns a structured error, not an exception."""

    async def test_unknown_tool_returns_error_dict(self, mcp_server: ArborMCPServer):
...
class TestToolInvocation:
    ...
    async def test_successful_invocation_returns_result(self, mcp_server: ArborMCPServer):
        result = await mcp_server.call_tool(
            "test_echo", company_id=TENANT_A, user_id=USER_A, message="ping"
        )
        assert result == {"echo": "ping", "company_id": TENANT_A}

    async def test_default_user_id_is_system(self, mcp_server: ArborMCPServer):
    ...
    async def test_failing_tool_returns_error_dict(self, mcp_server: ArborMCPServer):
    ...
    async def test_tool_receives_tenant_context(self, mcp_server: ArborMCPServer):
...
class TestAuditLogging:
    ...
    async def test_successful_call_creates_audit_entry(self, mcp_server: ArborMCPServer):
    ...
    async def test_failed_call_creates_audit_entry_with_error(self, mcp_server: ArborMCPServer):
    ...
    async def test_audit_log_filtered_by_company(self, mcp_server: ArborMCPServer):
    ...
    async def test_audit_log_limit(self, mcp_server: ArborMCPServer):
        for _ in range(5):
            await mcp_server.call_tool("test_echo", company_id=TENANT_A, user_id=USER_A)
        log = mcp_server.get_audit_log(limit=3)
        assert len(log) == 3

    async def test_audit_log_sorted_newest_first(self, mcp_server: ArborMCPServer):
    ...
    async def test_audit_entries_do_not_contain_request_payload(self, mcp_server: ArborMCPServer):
    ...
    async def test_audit_entry_has_unique_id(self, mcp_server: ArborMCPServer):
        await mcp_server.call_tool("test_echo", company_id=TENANT_A, user_id=USER_A)
        await mcp_server.call_tool("test_echo", company_id=TENANT_A, user_id=USER_A)
        log = mcp_server.get_audit_log()
...
class TestHealthEndpoint:
    ...
    def test_health_includes_server_name(self, mcp_server: ArborMCPServer):
        health = mcp_server.get_health()
        assert health["server"] == "arbor-test-server"

    def test_health_includes_version(self, mcp_server: ArborMCPServer):
        health = mcp_server.get_health()
        assert health["version"] == "0.0.1"

    def test_health_reports_tool_count(self, mcp_server: ArborMCPServer):
        health = mcp_server.get_health()
        assert health["tools_registered"] == 3

    async def test_health_tracks_total_invocations(self, mcp_server: ArborMCPServer):
    ...
    def test_health_includes_timestamp(self, mcp_server: ArborMCPServer):
        health = mcp_server.get_health()
...
class TestResourceRegistration:
    ...
    def test_register_and_list_resource(self):
        server = ArborMCPServer(name="res-test")

        @server.resource("arbor://status", name="Status", description="Server status")
        def status_resource():
            return {"status": "ok"}

        resources = server.list_resources()
        assert len(resources) == 1
        assert resources[0]["uri"] == "arbor://status"
        assert resources[0]["name"] == "Status"

    def test_get_resource_by_uri(self):
        server = ArborMCPServer(name="res-test")

        @server.resource("arbor://version")
        def version_resource():
            return {"version": "1.0.0"}

        result = server.get_resource("arbor://version")
        assert result == {"version": "1.0.0"}

    def test_get_unknown_resource_returns_none(self):
        server = ArborMCPServer(name="res-test")
        assert server.get_resource("arbor://nonexistent") is None

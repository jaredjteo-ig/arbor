...
## Arbor Project Context

This repository is **Arbor** — an AI-powered HRIS (Human Resource Information System) with employment law advisory for Singapore SMEs, built on the Kailash SDK and open-sourced under Terrene Foundation.

### Project-Specific Agents (`agents/project/`)

- **arbor-platform-specialist** — Overall platform architecture, 23+ routers, 60+ models
- **arbor-web-specialist** — Next.js/React frontend
- **arbor-mobile-specialist** — Flutter mobile app
- **sg-payroll-expert** — Payroll engine (zero LLM, deterministic)
- **sg-employment-law-expert** — 6 regulatory domains
- **hr-calculator-specialist** — 7 SG HR calculators
- **advisory-safety-chain-specialist** — 13-step advisory pipeline
- **kb-pipeline-specialist** — Knowledge base and search
- **trust-governance-specialist** — EATP/CARE governance

### Project-Specific Skills (`skills/project/`)

See `skills/project/SKILL.md` for the full index covering: HRIS engine (12 modules), platform architecture, employment law, advisory safety chain, trust governance, calculators, KB pipeline, auth/security, **security-patterns** (60 codified patterns P1-P60 from rounds 12-7 + obayashi 1/2/polish + engagement post-walk polish + P4 wave May 2026 — saga compensation, hash-chained audit log, cache invalidation, per-tenant locks, idempotency, soft-delete, cost cap, prompt-injection sanitization, OAuth user_id binding, syncToken protocol, cron-via-docker-exec, role-aware UX gating, default-deny caches, LLM transient-failure UX, **permission predicate vs role proxy (P49)**, **privacy asymmetry — data subjects can re-identify their own pseudonymous data (P50)**, **aggregate stats are necessary but not sufficient (P51)**, **demo seed realism via probability-weighted draws (P52)**, **conditional explanatory copy (P53)**, **decrypt-at-export boundary for field-encrypted PII (P54)**, **manager-of-team scope derived from `Employee.reporting_manager_id`, NOT a 4th role (P55)**, **action-vs-analytics scope asymmetry (P56)**, **self-approval block on every role (P57)**, **hash-chained audit log alongside the mutable record; audit failure never blocks (P58)**, **FastAPI route ordering: literals before /{id} (P59)**, **DataFlow read() fallback to filtered list() (P60)**), **engagement-surveys** (three-tier anonymity, HMAC pseudonyms with versioned secrets, action loop closing back to employees, manager view depth with self-exclusion + per-question + 6-pulse trend, Z01-Z44 amendment catalogue), documents, company/user management, and MCP integrations.

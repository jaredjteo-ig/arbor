...
**If someone asks a Thailand-specific question during the demo, the system will flag it as requiring escalation.** This would make Arbor look like it is rejecting the audience's use case.
...
- [ ] Verify `arbor.terrene.foundation` returns 200 on `/api/health`
- [ ] Verify SSL certificate is valid
- [ ] Check server uptime — run `curl -s https://arbor.terrene.foundation/api/health | python3 -m json.tool`
- [ ] Deploy latest code if needed — 5 commits since last deploy include SSE fixes and KB search fallback improvements

### API & Budget

- [ ] Verify OpenAI API key on server is valid and has credits
- [ ] Have a backup BYOK key ready (different provider if possible)
...
- [ ] Create demo company with realistic name (e.g. "Sakura Trading Pte Ltd" — Singapore company to match the KB)

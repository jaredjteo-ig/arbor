...
def _get_attendance_settings(company_id: int) -> dict:
    ...
    settings = dataflow_crud.list_records("AttendanceSettings",
...
async def clock_in(
    ...
    existing = dataflow_crud.list_records("AttendanceRecord",
    ...
    record = dataflow_crud.create("AttendanceRecord",
        {
            "employee_id": emp["id"],
            "company_id": company_id,
            "date": today,
...
async def clock_out(
    ...
    dataflow_crud.update("AttendanceRecord",
        record["id"],
        {
            "clock_out": now,
...
async def get_today_record(
    ...
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    records = dataflow_crud.list_records("AttendanceRecord",
        {"employee_id": emp["id"], "date": today},
        limit=1,
    )
...
async def update_attendance_settings(
    ...
    existing = dataflow_crud.list_records("AttendanceSettings",
        {"company_id": company_id},
        limit=1,
    )
    if existing:
...
async def submit_timesheet(
    ...
    existing = dataflow_crud.list_records("TimesheetApproval",
    ...
    now = datetime.now(timezone.utc).isoformat()
    timesheet = dataflow_crud.create("TimesheetApproval",
        {
            "employee_id": emp["id"],
            "company_id": company_id,
            "month": month,
...
async def approve_timesheet(
    ...
    dataflow_crud.update("TimesheetApproval",
        timesheet_id,
        {
            "status": "approved",
...
def _get_lateness_settings(company_id: int) -> dict:
    ...
    settings = dataflow_crud.list_records("LatenessSettings",
        {"company_id": company_id},
        limit=1,
    )
    if settings:
...
def _get_early_departure_settings(company_id: int) -> dict:
    ...
    settings = dataflow_crud.list_records("EarlyDepartureSettings",
        {"company_id": company_id},
        limit=1,
    )
    if settings:
...
async def update_lateness_settings(
    ...
    existing = dataflow_crud.list_records("LatenessSettings",
        {"company_id": company_id},
        limit=1,
    )
    if existing:
...
async def update_early_departure_settings(
    ...
    existing = dataflow_crud.list_records("EarlyDepartureSettings",
        {"company_id": company_id},
        limit=1,
    )
    if existing:
...
async def aggregated_summary(
    ...
    all_records = dataflow_crud.list_records("AttendanceRecord",
        {"company_id": company_id},
    )

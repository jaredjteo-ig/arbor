...
def _audit_claim(
    ...
    return dataflow_crud.create("ClaimAuditEntry",
        {
            "claim_id": claim_id,
...
async def list_claim_categories(
    ...
    categories = dataflow_crud.list_records("ClaimCategory",
...
async def create_claim_category(
    ...
    category = dataflow_crud.create("ClaimCategory",
        {
            "company_id": company_id,
            "name": name,
...
async def create_claim(
    ...
    claim = dataflow_crud.create("Claim",
        {
            "employee_id": emp["id"],
            "company_id": company_id,
...
async def pending_for_payroll(
    ...
    claims = dataflow_crud.list_records("Claim",
...
async def add_claim_item(
    ...
    if monthly_limit > 0:
        ...
        month_claims = dataflow_crud.list_records("Claim",
            {"employee_id": emp["id"], "claim_month": claim.get("claim_month", "")},
        )
        month_total = 0.0
        for mc in month_claims:
            items = dataflow_crud.list_records("ClaimItem",
    ...
    item = dataflow_crud.create("ClaimItem",
        {
            "claim_id": claim_id,
            "company_id": company_id,
...
async def create_claim_group(
    ...
    group = dataflow_crud.create("ClaimGroup",
        {
            "employee_id": emp["id"],
            "company_id": company_id,
            "name": name,
...
async def get_payroll_ready_claims(
    ...
    enriched = []
    for c in ready:
        emp_records = dataflow_crud.list_records("Employee", {"id": c.get("employee_id")}, limit=1
        )

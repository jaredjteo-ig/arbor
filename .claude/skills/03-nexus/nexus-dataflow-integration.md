...
> **DataFlow v0.11.0**: The parameters `enable_model_persistence`, `skip_migration`, and `existing_schema_mode` have been **removed**. The only critical Nexus-side setting is `auto_discovery=False`. DataFlow's `auto_migrate=True` (default) now works correctly in Docker/FastAPI via synchronous DDL operations.
...
db = DataFlow(
    ...
    auto_migrate=True,  # Default - works in Docker/FastAPI via synchronous DDL operations
)
...
- Uses the synchronous DDL executor for synchronous DDL operations
...
- Automatic schema migration via synchronous DDL operations
- Fast startup

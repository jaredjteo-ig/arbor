...
| `security-patterns.md`       | Reusable patterns         | 17 codified patterns from round-12/13/14: saga compensation, hash-chained audit log, cache invalidation, per-tenant locks, idempotency window, soft-delete, cost cap, prompt-injection sanitization, OAuth user_id binding, syncToken protocol, cron-via-docker-exec. Use when implementing the matching shape — these are post-audit closure forms, not suggestions.                                                                                                                                                                                                      |

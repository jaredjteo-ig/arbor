...
1. **Diff analysis** — Compare `main` against the last release tag for each package:
   ...
   ```
   git log <last-tag>..HEAD --            → Core SDK changes?
   git log <last-tag>..HEAD --   → DataFlow changes?
   git log <last-tag>..HEAD --     → Kaizen changes?
   git log <last-tag>..HEAD --      → Nexus changes?
   ```
...
| the SDK source | `__version__ = "X.Y.Z"` | `__version__ = "0.13.0"` |
...
| File                                                 | Field                              |
| ---------------------------------------------------- | ---------------------------------- |
| the package source           | `version = "X.Y.Z"`                |
| the package source                 | `version="X.Y.Z"`                  |
| the package source | `__version__ = "X.Y.Z"`            |
| the package source           | `dependencies: kailash>=A.B.C`     |
| the package source                 | `install_requires: kailash>=A.B.C` |
...
2. For frameworks: `cd  && python -m build`
...
| ---------------- | ------------------------------------------ | ------------------------------------ | ---------------------------------------------------- | ----------- |
| kailash          | `pyproject.toml`                           | `setup.py`                           | the SDK source                            | —           |
| kailash-dataflow | the package source | the package source | the package source | `kailash>=` |
| kailash-kaizen   | the package source   | the package source   | the package source     | `kailash>=` |
| kailash-nexus    | the package source    | the package source    | the package source       | `kailash>=` |

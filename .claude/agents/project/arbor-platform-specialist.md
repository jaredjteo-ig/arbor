...
| Export to external system reads encrypted-at-rest PII  | P54 decrypt at the export boundary; bytes test asserts no `gAAAAA` Fernet prefix                                 | `test_p0_statutory_pii_decrypt.py`                                              |
| "Line manager can act on team" surface                 | P55 derive from `Employee.reporting_manager_id` via `services/manager_scope.py` — do NOT add a 4th role          | `test_manager_scope.py` + `test_p4_mg_2_team_scope.py`                          |
| Action-vs-analytics scope on a team surface            | P56 direct-only for verbs (approve/reject), direct+indirect for aggregates (engagement)                          | `test_p4_mg_5_team_engagement.py`                                               |
| Approve/reject endpoint with role check                | P57 self-approval block before role check — owner can NOT approve own action                                     | `test_p4_mg_2_team_scope.py`                                                    |
| HR-decision endpoint mutating `reviewed_by`            | P58 `_audit_*` helper appends `<entity>.<action>` to hash-chained `AuditLogEntry`; failure logs but never raises | `test_redteam_round2_audit_log.py`                                              |
| Router mixing static path + `/{id}` path               | P59 register literal route before `/{id}` or FastAPI 422s the literal                                            | `test_p4_mg_4_team_appraisals.py::test_mg4_to_review_registered_before_id_path` |
| Single-record DB read that intermittently returns None | P60 `dataflow_crud.read()` falls back to `express_sync.list({"id": id})` automatically                           | `test_p0_statutory_pii_decrypt.py::test_p0_4_*`                                 |

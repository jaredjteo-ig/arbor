  ...
  async function onSubmit(data: ResetPasswordFormValues) {
    ...
    try {
      ...
      if (error instanceof AuthError) {
        setServerError(t("auth.reset_failed"));
      } else {
        ...
        <>
          <div className="text-center mb-8">
            ...
            <p className="mt-1 text-sm text-[var(--color-gray-500)]">
              {t("auth.reset_password_subheading")}
            </p>
          </div>
          ...
          {!token && (
            <div
              role="alert"
              className="mb-4 rounded-[8px] border-l-4 border-[var(--color-warning)] bg-[var(--color-warning-bg)] px-4 py-3"
            >
      ...
      fallback={
        <div className="rounded-[12px] bg-[var(--color-surface-card)] shadow-[var(--shadow-raised)] p-8">
          <div className="flex justify-center">
            <div
              className="h-8 w-8 animate-spin rounded-full border-4 border-[var(--color-gray-200)] border-t-[var(--color-primary)]"
              role="status"
            >
